...
const InstalledCard = ({ installedApp }) => {
  ...
  return (
    <div className="flex items-center justify-between p-4 my-4 shadow-[0_0_15px_rgba(9,9,9,0.2)] rounded-2xl border-[#6a728250] border-1">
      <Link to={`/apps/${installedApp.id}`} className="flex flex-1 items-center space-x-4">
        <div className="h-20 w-20 flex justify-center items-center rounded-2xl overflow-hidden relative bg-[#0f0f0f20]">
          <img
            src={image}
            alt={title}
            onLoad={() => setImgLoaded(true)}
            className={`object-contain max-h-full max-w-full rounded-2xl transition-opacity duration-200 ${
