...
const InstalledCard = ({ installedApp }) => {
  ...
  return (
    <div className="bg-white flex items-center justify-between p-4 my-4 shadow-[0_0_15px_rgba(9,9,9,0.2)] rounded-2xl">
      <Link to={`/apps/${installedApp.id}`}>
        <div className="flex items-center flex-1 space-x-4">
          <div className="h-20 w-20 flex justify-center items-center bg-gray-50 rounded-2xl overflow-hidden">
            <img
              src={image}
              alt={title}
          ...
          <div className="flex flex-col justify-center w-full">
            ...
            <div className="flex space-x-3 mt-2">
              <span className="bg-[#f1f5e8] px-3 py-1 rounded-lg flex items-center space-x-1 text-[#00D390]">
                <FontAwesomeIcon icon={faDownload} />
                <span>{numeral(downloads).format('0.0a').toUpperCase()}</span>
              </span>
              <span className="bg-[#fff0e1] px-3 py-1 rounded-lg flex items-center space-x-1 text-[#FF8811]">
                {ratingAvg}
              </span>
            </div>
          </div>
